...
BEGIN
	...
	SET NOCOUNT ON;
		...
		if (@INSERTCOUNT >0 AND @DELETEDCOUNT > 0)
			...
			if (@INSERTCOUNT >0 AND @DELETEDCOUNT=0)
				BEGIN
					INSERT INTO BBCT.T_HISTORY (DATE_DONE, TABLE_NAME, ACTIVITY, DATA_FROM, DATA_TO, DESCRIPTION, USERNAME) VALUES (CURRENT_TIMESTAMP, 'T_SEAL_STICKER_PAIR', 'I', null, @INSERTEDCOVERPAGENO+ ', ' + @INSERTEDSTICKERNO, 'Inserted ' + @INSERTEDCOVERPAGENO+ ' and ' + @INSERTEDSTICKERNO + ' seal-sticker pair', 'UF200694');		
				END
			else 
			if (@INSERTCOUNT=0 AND @DELETEDCOUNT > 0)
			BEGIN
				INSERT INTO BBCT.T_HISTORY (DATE_DONE, TABLE_NAME, ACTIVITY, DATA_FROM, DATA_TO, DESCRIPTION, USERNAME) VALUES (CURRENT_TIMESTAMP, 'T_SEAL_STICKER_PAIR', 'D', @DELETEDCOVERPAGENO + ', ' + @DELETEDSTICKERNO, null, 'Deleted ' + @DELETEDCOVERPAGENO + ' and ' + @DELETEDSTICKERNO + ' seal-sticker pair', 'UF200694');
			END
END
